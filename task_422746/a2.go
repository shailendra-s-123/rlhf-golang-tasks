...
func (cp *ConnPool) cleanupConnections() {
	for {
		...
		now := time.Now()
		for i, conn := range cp.idleConnections {
			lastUse := conn.(*connection).lastUse
			if now.Sub(lastUse) > connectionTTL {
...
// connection is a wrapper around net.Conn to track its last use time
